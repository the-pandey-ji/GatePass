...
<%
	Connection conn1 = null;
		...
		ResultSet rs1 = st1.executeQuery("select max(id) from visitor"); // Executing the Query
		while(rs1.next()){
    		System.out.println("Select max id in - Print.jsp page");
    	id=rs1.getInt(1);
        }
%>
...
<%
	...
	String ip = db.getServerIp();
		...
		while(rs.next()){
    		System.out.println("Select Image for max id in - "+rs.getString(11));
%>
...
<input type="submit"  name="print" value="print" style="position:absolute;left:425px;top:350px;width:75px;height:24px;font-family:Arial;font-size:13px;z-index:68" onclick="window.print();">
</body>
<% } rs.close();conn.close();%>
</html>